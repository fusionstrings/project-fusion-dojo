...
var profile = {
	...
	resourceTags: {
		...
		// All files in the `win/resources` directory that are not CSS files are marked as copy-only, since these files
		...
		copyOnly: function (filename, moduleId) {
			return (/^fuse-portlet\/resources\//.test(filename) && !/\.css$/.test(filename));
		},
		...
		amd: function (filename, moduleId) {
			return !this.copyOnly(filename, moduleId) && /\.js$/.test(filename);
		},
		...
		miniExclude: function (filename, moduleId) {
			return (/^fuse-portlet\/tests\//).test(moduleId) || moduleId in {
				'yo-dojo/package': 1
			};
		}
	}
